...
#define INSERT_INDEX_USAGE_HEAD_SQL                                            \
  "INSERT INTO "                                                               \
  "__all_index_usage_info(tenant_id,table_id,object_id,total_exec_count,"      \
  "start_used,last_used,last_flush_time) VALUES"
  ...
  if (!result_list.empty()) {
    ...
    for (common::ObList<ObIndexUsageInfoMgr::ObIndexUsagePair,
      ...
      insert_update_sql.append_fmt(
          "(%lu,%lu,%lu,%lu,now(6),usec_to_time(%ld),now(6))",
