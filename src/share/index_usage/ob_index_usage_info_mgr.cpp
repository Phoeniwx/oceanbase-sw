...
int ObIndexUsageInfoMgr::init() {
  ...
  if (OB_UNLIKELY(is_inited_)) {
    ...
    LOG_WARN("init twice", K(ret));
                 ...
                 index_usage_map_.create(DEFAULT_MAX_HASH_BUCKET_CNT, attr))) {
...
int ObIndexUsageInfoMgr::update(const uint64_t database_id,
  ...
  ObIndexUsageKey key(database_id, tenant_id, index_table_id);
  ObIndexUsageOp update_op(ObIndexUsageOpMode::UPDATE);
  ObIndexUsageInfo tmp_info;
  SpinRLockGuard guard(destory_lock_);
  if (OB_FAIL(index_usage_map_.get_refactored(key, tmp_info))) {
    if (OB_LIKELY(ret == OB_HASH_NOT_EXIST)) {
      if (OB_FAIL(index_usage_map_.set_refactored(
              key, ObIndexUsageInfo(key.index_table_id, 0, time(NULL))))) {
        // other thread set the refactor, try update again
        if (OB_FAIL(index_usage_map_.get_refactored(key, tmp_info))) {
          LOG_WARN("failed to get refactored", K(ret));
        } else {
          if (OB_FAIL(index_usage_map_.atomic_refactored(key, update_op))) {
...
int ObIndexUsageInfoMgr::sample(
  ...
  for (ObIndexUsageHashMap::iterator it = index_usage_map_.begin();
    ...
    if (sample_count < map_size &&
        rand() % (map_size - index) >= sample_count) {
      continue;
    }
...
int ObIndexUsageInfoMgr::del(ObIndexUsageKey& key) {
  ...
  if (OB_ISNULL(info_ptr_ptr)) {
    ...
    if (OB_FAIL(index_usage_map_.erase_refactored(key))) {
...
/*
int ObIndexUsageInfoMgr::alloc_new_record(const ObIndexUsageKey &temp_key,
                                          ObIndexUsageKey *key,
  ...
  if (OB_ISNULL(buf_key) || OB_ISNULL(buf_info)) {
    ...
    key = new (buf_key) ObIndexUsageKey(
        temp_key.database_id, temp_key.tenant_id, temp_key.index_table_id);
    info =
        new (buf_key) ObIndexUsageInfo(temp_key.index_table_id, 0, time(NULL));
  }
  if (OB_FAIL(ret)) {
