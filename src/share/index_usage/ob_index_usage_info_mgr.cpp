...
int ObIndexUsageInfoMgr::update(const int64_t database_id,
  ...
  if (OB_FAIL(index_usage_map_.atomic_refactored(&temp_key, update_op))) {
    ...
    if (ret == OB_HASH_NOT_EXIST) {
      ...
      } else if (OB_FAIL(index_usage_map_.set_or_update(new_key, new_info,
                                                        update_op))) {
...
int ObIndexUsageInfoMgr::del(ObIndexUsageKey *key) {
  ...
  SpinRLockGuard guard(destory_lock_);
  ObIndexUsageInfo *const *info_ptr_ptr =
      index_usage_map_.get(const_cast<ObIndexUsageKey *const>(key));
  if (OB_ISNULL(info_ptr_ptr)) {
    LOG_WARN("index usage key not exists");
  } else {
    if (OB_FAIL(index_usage_map_.erase_refactored(
            const_cast<ObIndexUsageKey *const>(key)))) {
